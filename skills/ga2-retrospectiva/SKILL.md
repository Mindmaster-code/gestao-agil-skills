---
name: ga2-retrospectiva
description: "Conduz uma retrospectiva sobre o modo de trabalhar e escolhe uma melhoria com dono e data. Use quando pedirem retro, manter, melhorar e parar, ou mudança de processo após um ciclo."
---
...
- `retrospectiva-kit-09` — Retrospectiva | Reflexões da Equipe (canvas do kit do curso) — padrão
- `retrospectiva-v6` — Canvas Editável — Retrospectiva (canvas editável v6)

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py retrospectiva-kit-09 --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py retrospectiva-kit-09 --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
