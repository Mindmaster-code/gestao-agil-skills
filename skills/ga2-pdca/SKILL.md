...
- `pdca-lab` — PDCA (laboratório) (desenhado no estilo do kit; não está no curso) — padrão

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py pdca-lab --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py pdca-lab --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
