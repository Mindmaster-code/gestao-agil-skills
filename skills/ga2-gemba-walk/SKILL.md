---
name: ga2-gemba-walk
description: "Conduz uma observação no lugar onde o trabalho acontece, com pergunta, fatos e uma ação. Use quando pedirem Gemba, observação direta, investigação no local ou entendimento de uma queda."
---
...
- `gemba-v6` — Canvas Editável — Roteiro do Vá e Veja (canvas editável v6) — padrão

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py gemba-v6 --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py gemba-v6 --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
