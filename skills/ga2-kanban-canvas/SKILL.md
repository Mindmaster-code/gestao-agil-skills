---
name: ga2-kanban-canvas
description: "Desenha um quadro Kanban com estados, raias, cartões, limites e regras visíveis. Use quando pedirem Kanban, quadro de trabalho, fluxo, cartões, bloqueios ou políticas do quadro."
---
...
- `kanban-kit-07` — Kanban | Quadro de Acompanhamento Simples (canvas do kit do curso) — padrão
- `kanban-avancado-kit-7-1` — Kanban | Quadro de Acompanhamento Avançado (canvas do kit do curso)
- `kanban-canvas-v6` — Kanban Canvas (jornada v6) (canvas editável v6)
- `kanban-politicas-kit-7-2` — Políticas do Kanban | "Combinados" da Equipe (canvas do kit do curso)

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py kanban-kit-07 --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py kanban-kit-07 --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
