---
name: ga2-causa-raiz
description: "Investiga causas com 5 Porquês ou Ishikawa e exige evidência em cada ligação. Use quando pedirem causa raiz, 5 Porquês, espinha de peixe, post-mortem ou separação entre sintoma e causa."
---
...
- `cinco-porques-b-06` — Causa raiz — 5 Porquês (editável) (editável por módulo) — padrão
- `ishikawa-lab` — Ishikawa — espinha de peixe (laboratório) (desenhado no estilo do kit; não está no curso)

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py cinco-porques-b-06 --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py cinco-porques-b-06 --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
