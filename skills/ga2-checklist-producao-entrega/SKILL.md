---
name: ga2-checklist-producao-entrega
description: "Define o que um trabalho precisa para começar e para ser entregue com prova. Use quando perguntarem se algo pode começar, se está pronto, ou citarem DoR, DoD e critério de pronto."
---
...
- `dor-dod-lab` — Portões DoR / DoD (laboratório) (desenhado no estilo do kit; não está no curso) — padrão

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py dor-dod-lab --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py dor-dod-lab --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
