---
name: ga2-plano-do-ciclo
description: "Planeja um ciclo com uma meta, capacidade segura, trabalhos com dono e data, e critério de pronto. Use quando pedirem plano da semana, sprint, meta do ciclo ou escolha do que entra."
---
...
- `plano-ciclo-v6` — Canvas Editável — Plano do Ciclo (canvas editável v6) — padrão
- `sprint-plano-ciclo-c` — Sprint / Plano do Ciclo (jornada) (folha da jornada)

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py plano-ciclo-v6 --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py plano-ciclo-v6 --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
