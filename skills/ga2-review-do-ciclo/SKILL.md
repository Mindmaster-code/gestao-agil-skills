---
name: ga2-review-do-ciclo
description: "Compara o combinado com o entregue, exige prova e registra a decisão seguinte. Use quando pedirem review, revisão do ciclo, demonstração da entrega ou fechamento da semana."
---
...
- `revisao-kit-08` — Revisão de Entregas | Debriefing (canvas do kit do curso) — padrão
- `revisao-v6` — Canvas Editável — Revisão do Ciclo (canvas editável v6)

Para gerar a partir do `.md` preenchido (Python 3.10 ou mais novo, na pasta do pacote):

```bash
python3 construtor/construir.py revisao-kit-08 --documento --md meu-caso.md --saida meu-caso.html
python3 construtor/construir.py revisao-kit-08 --canvas --md meu-caso.md --saida meu-caso-canvas.html
```
